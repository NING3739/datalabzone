...
It provides a comprehensive set of tools and libraries that make it a preferred choice among data scientists, statisticians, and researcher[^1]. It provides a comprehensive set of tools and libraries that make it a preferred choice among data scientists, statisticians, and researcher.
...
murders$population #<5>

names(murders) #<6>

pop <- murders$population #<7>
length(pop) #<7>

class(pop) #<8>
class(murders$state) #<8>
...
class(murders$region) #<10>

levels(murders$region) #<11>
```

1.  Loading the dslabs package and the murders dataset;
2.  Determining that the murders dataset is of the "data frame" class;
3.  Finding out more about [the structure of the object]{style="color:red"};
4.  Showing [the first 6 lines of the dataset]{style="color:red"};
5.  Using the accessor operator [\$]{style="color:red"} to obtain the population column;
6.  Displaying [the variable names]{style="color:red"} in the murders dataset;
7.  Determining [how many entries are in a vector]{style="color:red"};
8.  Vectors can be of class numeric and character;
9.  Logical vectors are either TRUE or FALSE;
10. Factors are another type of class;
11. Obtaining [the levels of a factor]{style="color:red"}.
